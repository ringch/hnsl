L.Marker.include({
    _initIcon: function () {
        ...
        L.DomUtil.setPosition(this._light, i)
        ...
        var i = this.options, t = "leaflet-zoom-" + (this._zoomAnimated ? "animated" : "hide");
        var e = i.icon.createIcon(this._icon), o = false;
        if (e !== this._icon) {
            if (this._icon) {
                this._removeIcon()
            }
        ...
        var s = i.icon.createShadow(this._shadow), n = false;
        if (s !== this._shadow) {
            this._removeShadow();
            n = true
        }
